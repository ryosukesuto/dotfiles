...
# quick-push - Codex-native commit & push

## 目的
現在の変更を素早くコミットしてリモートにプッシュする。Codex自身が差分確認、対象ファイル選定、commit/pushを行い、別paneや別agentは起動しない。
...
if [[ "$current_branch" == "main" ]] || [[ "$current_branch" == "master" ]]; then
    ...
    if [[ "$allow" == "true" ]]; then
        ...
        # worktree作成を提案する（AGENTS.md/CLAUDE.mdのworktree必須ルール）
        echo "main直コミットはブロックされています。worktreeを作成します"
    fi
fi
```

なぜこのチェックが必要か: AGENTS.md/CLAUDE.mdには「ブランチ作業はworktree必須」と書かれていることがあるが、dotfilesなど個人リポジトリでは `wt.allowDirectCommit=true` でopt-outされている。ルールの禁止側だけ見てworktree作成に進むと、opt-outされたリポジトリで無駄なworktreeを作ってユーザーを待たせる。**worktreeフローに入る前に必ずこの設定を確認する**。
...
判断基準:
- 公式公開URL（`https://www.example.com/corporate/...` 等）や組織名のみの言及（`WinTicket org`）は許容
...
- `wt.allowDirectCommit=true` のリポジトリ（dotfiles等）でworktree作成を提案しない。AGENTS.md/CLAUDE.mdの「worktree必須」はopt-out前提のルール。main/masterブランチに入ったら即座に worktree を作ろうとせず、先に `git config --get wt.allowDirectCommit` を確認する
- Codexでは権限とsandboxは現在のセッション設定に従う。Claude Code専用のsandbox無効化指定は使わない
- `git push` が `Permission denied (publickey)` で失敗した場合は、ネットワーク制限より先にSSH鍵・remote URL・`gh auth status` を確認する
