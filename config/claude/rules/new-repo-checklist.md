...
4. チーム権限割り当て（5分）
5. branch protection 設定（5分）
6. 初回 harness-audit でベースライン取得（10分）
7. 依存更新自動化（Renovate）導入（5分）
8. 完了後の運用サイクル登録（任意）
```
...
リポジトリ作成直後は作成者のみが Admin。そのままだと作成者が不在になった瞬間にメンテナンス不能になるため、運用チームに権限を割り当てる。

### 決めること

- このリポジトリのオーナーチームは誰か（例: server / infra / qa）
- 該当チームの admin / 開発者 / ゲストの分け方が org で定義されているか
...
| Team | Role | 用途 |
|------|------|------|
| `<team>-admin` | Admin | 設定変更・branch protection 更新・人の出し入れ |
| `<team>` | Write | 日常の push / PR / レビュー |
| `<team>-guest` | Write（任意） | 業務委託・ヘルプ要員などスコープが狭いメンバー |

Admin を Team 単位で付与するのが原則。個人ユーザーへの直接 Admin 付与は属人化するため避ける。
...
# 必要なら
gh api -X PUT /orgs/$ORG/teams/$TEAM-guest/repos/$ORG/$REPO -f permission=push
```

`permission=push` は Write 相当。`admin` / `push` / `pull` / `triage` / `maintain` から選ぶ。

org 固有のチーム名・admin 命名規則は `dotfiles-private/config/claude/rules/<org>-repo-permissions.local.md` を参照。
...
`vulnerabilityAlerts.prPriority` は `renovate-config-validator` で schema warning（`prPriority` の parent は `packageRules` のみという定義）が出るが、Renovate は `vulnerabilityAlerts` オブジェクトを vulnerability 更新の config に直接マージするため機能上は動く。warning を消すために `packageRules` に `matchUpdateTypes: ["security"]` の entry を切って移すのは誤り — Renovate に `security` という `UpdateType` は存在せず、このルールは永久にマッチしないため優先度ブーストが黙って無効化される（詳細は `winticket-dep-updates.local.md` 参照）。

org レベルで Renovate App が install されていない場合は org admin に依頼。設定ファイル追加だけでは動かない。

PR レビュー自動化の `allowed_bots` は default (空) で運用する。`dependabot[bot]` / `renovate[bot]` のいずれも入れない。Wiz custom rule `459cd9a5-6932-401a-b31a-f7d9eb6b3c5a` (`claude-code-action should not use wildcard or dependabot in allowed_bots`, HIGH) は dependabot と renovate の両方を Confused Deputy 攻撃ベクトルとして検出するため。bot PR への自動レビューは諦め、Renovate 自身の Release Notes と人間レビューでカバーする。あわせて `exclude_comments_by_actor: "*[bot]"` で bot コメントを Claude のコンテキストから除外する。
