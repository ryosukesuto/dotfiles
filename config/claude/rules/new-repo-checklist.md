...
PR レビュー自動化の `allowed_bots` には `renovate[bot]` を入れる（依存更新 PR にもレビューが走る）。`dependabot[bot]` は Wiz custom rule `459cd9a5-6932-401a-b31a-f7d9eb6b3c5a` (`claude-code-action should not use wildcard or dependabot in allowed_bots`, HIGH) で禁止のため入れない。WinTicket org は Renovate に統一済みなので運用上の不便はない。
