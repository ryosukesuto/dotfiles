...
- codex-review使用時はtmuxセッション内またはcmuxターミナル内で実行すること

## 基本的な使い方

環境に応じて自動的に切り替え:
- tmux/cmux内: codex-review（右ペインでインタラクティブに対話）
- それ以外: codex exec（単発実行）

### tmux/cmux内: codex-review

```bash
PANE_MGR=~/.claude/skills/codex-review/scripts/pane-manager.sh

$PANE_MGR ensure                           # Codexペインを作成
$PANE_MGR send "git diffをレビューして"     # メッセージ送信
$PANE_MGR wait_response 180 && $PANE_MGR capture 200  # 応答待機とキャプチャ
```
...
Skill/ルール等の設計が大きいタスクでは、CodexにPlan作成を任せてClaudeがレビューで絞る。
Codexはオーバーエンジニアリングしがち、Claudeはそれを指摘できる。

1. Codexに設計メモを渡してPlan作成（Codex = 司令塔）
2. Claude Codeでレビュー（過剰な複雑さを指摘）
3. 指摘をCodexにコピペして修正させる
4. 収束するまで2-3の往復（根本的に間違った方向に進んでいないかざっと確認）
5. Claude Codeで実装（配置、既存パターンへの整合）
6. Codexで最終レビュー（抜け漏れ・エッジケース指摘）
7. Claude Codeで修正反映
