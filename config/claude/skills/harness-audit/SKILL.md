...
### 4. ハーネスルール台帳の棚卸し（追加と同じ優先度で実行する）

ハーネスは追加だけだとラチェット化（単調増加）して摩擦コストが膨らむ。監査の毎回、既存ルールの「降格・撤去」を必ず検討する。これは新しい改善の追加と同じ優先度で扱う。

#### 4.1 期限切れルールの抽出

リポジトリ内のルールメタ情報（`expires` / `downgrade_if`）を持つ箇所を洗い出す。`docs/harness-rules.md` の台帳がある場合はそれを起点にする。台帳がない場合は以下を grep する。

```bash
# YAML front matter / コメント形式の expires を一括抽出
grep -rE '^\s*#?\s*expires:\s*[0-9]{4}-[0-9]{2}-[0-9]{2}' \
  CLAUDE.md .claude/ docs/ .github/ .lefthook.yml 2>/dev/null
```

`expires <= today` のルールを一覧化する。
...
| 降格 | 再発は止まったが不安が残る、誤検知率が許容範囲 | 1段階下げる（CI → hook → linter → docs） |
| 撤去 | 再発 0 件、摩擦の方が大きい、設計側で吸収済み | 削除 |

降格の指針: ルール追加時に書いた `downgrade_if` 条件を満たしているかを確認する。`downgrade_if` がないルールは「追加時に降格条件を考えていなかった」=ハーネス設計の負債なので、棚卸し時に必ず追記する。

#### 4.3 メタ情報のないルールの整備

`expires` を持たないルールが見つかった場合、棚卸し時にメタ情報を後付けする。

```yaml
rule: <何を強制しているか>
reason: <なぜ必要か。再発した失敗類型 or commit hash>
added: <推測または棚卸し日>
expires: <棚卸し日 + 3-6ヶ月>
owner: <判断責任者>
downgrade_if: <どの観測で1段階下げるか>
```

詳細は `${CLAUDE_SKILL_DIR}/patterns.md` の P3 を参照。

#### 4.4 リポジトリ tier の確認

このリポジトリは `full` / `light` / `scratch` のどれに該当するかを確認する。tier に対して過剰なハーネスを持っていないかを点検する（例: scratch リポに CI gate が積んである等）。

### 5. 改善実行（ユーザー承認後）

レポート提示と棚卸し結果の提示後、ユーザーの承認を得てから改善を実行する。
...
新規ルール追加時の必須チェック:
- `rule` / `reason` / `added` / `expires` / `owner` / `downgrade_if` の6項目を併記する
- 「四半期ごとに1本足すより1本消す」を優先する。撤去候補がない場合は新規追加を見送ることも検討する
