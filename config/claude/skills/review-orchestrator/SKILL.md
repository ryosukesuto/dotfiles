...
| `schemas/finding.schema.json` | reviewer出力findingのJSON Schema。著者向け`review_draft`も保持 |
...
| `scripts/report.py` | normalize出力 → 内部優先度を隠した著者向けMarkdownレポートを自動生成 |
...
| `proofread` | 最終レポートとGitHub投稿草案のPRレビュー向け文体チェック |
...
uv run python3 ~/.claude/skills/review-orchestrator/scripts/report.py \
  ...
  -o /tmp/review.md
python3 ~/.claude/skills/proofread/check.py /tmp/review.md --pr-review
cat /tmp/review.md
```

最終レポートは各指摘に対象箇所・詳細・根拠・修正の方向・レビュー草案を含める。内部用の優先度、reviewer名、confidence、triage・dedupe・裁定過程は表示しない。文体チェックでPRレビュー固有の違反が出た場合は、`review_draft`を直して再生成し、違反が0になってから`/tmp/review.md`全文を返す。
...
`sources`と`rejected_alternatives`は裁定の監査用データとしてJSONにだけ保持する。report側では表示せず、採用された指摘の説明と著者向け草案だけを出す。
