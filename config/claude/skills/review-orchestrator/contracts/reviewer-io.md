...
reviewer 段階の仕事はフィルタではなく coverage（網羅）。確信が持てないもの・低 severity のものも含めて、発見した issue は全て報告する。件数で切り捨てない。

- severity 別のハードな最大件数キャップは設けない。reviewer 側で「上位のみ採用」して落とすことはしない（落とすと recall が下がり、後段で拾い直せない）
- 各 finding には `confidence`（0.00〜1.00）と `severity`（`must-fix` / `should-fix` / `watch`）を必ず付与する。確信度が低いものは低い `confidence` を付けて報告すればよい
- `severity`と`confidence`は並び替え・判定用の内部データであり、ユーザー向けレポートやGitHubコメントには表示しない
- 重複排除・重要度ランク付け・件数調整・レポートへの上位採用は、後段の normalizer / report が担う。reviewer は原則すべての finding を返す
- レポート肥大が懸念される場合も、絞り込みは normalize 段階で上位を採用する責務とし、reviewer 段階では行わない
...
4. `fix_hint`は`must-fix`では推奨、`watch`では任意。修正方針を常体で書き、`〜ください`は使わない
5. `review_draft` は必須。GitHubへそのまま貼れる2〜4文で、事実→理由→お願いの順に書く
6. `review_draft` に内部優先度、reviewer名、confidence、triage・dedupeなどのレビュー過程を書かない
7. 打消し文（「ただし実際には問題にならない」等）は禁止
...
- `finding_id`のprefixが`source_reviewer`と一致し、末尾が6文字の16進識別子であること
- `entity_key` のフォーマット正しさ（`{repo}:{path}(:{symbol})?`）
- `evidence.excerpt` が空文字でないこと
- `review_draft` が空文字でないこと
...
| review_draft | string | ✓ | 2〜4文。事実→理由→お願いの順。内部優先度・reviewer名・confidence・レビュー過程は禁止 |
...
- 件数で finding を切り捨てること禁止（severity 別の上限は設けない。確信が持てないもの・低 severity のものも報告し、絞り込みは後段の normalizer に任せる）
...
| `review-server` | クリーンアーキテクチャ、サーバー側規約、DBトランザクション |
