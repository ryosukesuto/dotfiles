...
`38ec876110f9fbf8b950c79f534430740c3ac009`（v1.0.101）を使用。

- `mcp__github_inline_comment__create_inline_comment` は v1.0.94 / v1.0.101 いずれも引き続き利用可能（過去の「v1.0.101 で削除」という記述は誤り）
- v1.0.94 で PR branch 上の `.claude/` と `.mcp.json` を base branch から自動 restore する防御が入った（attacker-controlled な指示ファイルを Claude に読ませない）。v1.0.101 でも継承されているため、新しい方を pin する。SKILL.md はこの挙動前提で書く（`.claude/skills/claude-code-review/SKILL.md` は base branch 版が読まれる）
...
1. action バージョンが v1.0.101 か（SHA `38ec8761...`）
