...
これを受けて org レベルで Auto-review on new commits（`triggerOnUpdates`）が OFF に切り替えられた。PR 作成時のレビューは従来通り走るが、push 後の再レビューは手動トリガー（Greptile の場合は `@greptileai`、Claude の場合は空コミット push）に寄せる運用。
...
50 review/seat の枠と Claude Code Review の Phase 2（事前収集 context ベースの一貫性チェック）の両立を踏まえ、リポを 3 tier に分類して Greptile 適用範囲を決める。Claude Code Review はどの Tier でも常に導入する。
...
- `pull_request` トリガー（opened / synchronize / reopened / ready_for_review）のみ
  - `issue_comment` トリガーは Wiz custom rule `CIWorkflow-020` 対応で削除済み。`@claude` 手動再レビューは空コミット push (`git commit --allow-empty -m "re-review" && git push`) で代替する
- force-push 検出: `git merge-base --is-ancestor` で判定、フル/増分を切り替え
- `allowed_bots`: default (空) で運用。Wiz rule `459cd9a5` (HIGH) で `dependabot` / `renovate` 双方が Confused Deputy 攻撃ベクトルとして検出されるため両方とも入れない。bot PR は人間レビュー
- `exclude_comments_by_actor: "*[bot]"`: bot コメントを Claude のコンテキストから除外（injection 経路の縮小）
- `--max-turns 50` 以上: SKILL.md が長いと 30 以下では `gh pr review` 投稿前に打ち切られる。`allowed_bots` 空化で bot 経由 Confused Deputy 攻撃面は遮断済みのため被害時間制約は不要
- `display_report: false`（untrusted な PR diff/description が Actions Summary に露出しないようにする。PR への投稿は SKILL.md の投稿ルールで完結する）
...
新モデル（`claude-opus-4-8` / `claude-sonnet-4-6` 等）を指定する場合は事前に動作確認すること。action / SDK 側の thinking API 互換性により `"thinking.type.enabled" is not supported for this model` で 400 エラーになるケースが過去にあった。

### action バージョン固定理由

`11a9dadd198803a0cea6bd53da3e0e8a762fc6ea`（v1.0.108）を使用。

- `mcp__github_inline_comment__create_inline_comment` は v1.0.94 以降継続して利用可能（過去の「v1.0.101 で削除」という記述は誤り）
- v1.0.94 で PR branch 上の `.claude/` と `.mcp.json` を base branch から自動 restore する防御が入った（attacker-controlled な指示ファイルを Claude に読ませない）。v1.0.108 でも継承されている。SKILL.md はこの挙動前提で書く（`.claude/skills/claude-code-review/SKILL.md` は base branch 版が読まれる）
- v1.0.102 で setup-bun が v2.2.0 に更新され Node.js 24 に対応（GitHub Actions の Node 20 EOL 2026-06 への先回り）
- v1.0.105 で Claude Code が生成する worktree ブランチ名（`feat+foo` 形式）の不具合が修正
...
Bash(git diff:*)                                   # 追加コミットレビューの差分取得
Bash(git log:*)                                    # コミット履歴確認
Read                                                # ファイル読み取り（PRブランチ / CONSISTENCY_CONTEXT_PATH）
```
...
`find` / `cat` / `grep` / `ls` の汎用 Bash 探索ツールと `WebSearch` / `WebFetch` は含めない（PF-2716 で削除。経緯は次節「allowedTools 縮小の経緯」参照）。

## allowedTools 縮小の経緯（PF-2716、Codex 指摘対応）

`WinTicket/ghas-notify` への導入 PR（PF-2715）で Codex レビューから受けた指摘を受けて、`claude-review.yml` を以下の3点で見直した。

### P0: allowedTools が広すぎる問題（token 漏洩経路）

以前の `--allowedTools` には `Bash(find:*)` / `Bash(cat:*)` / `Bash(grep:*)` / `Bash(ls:*)` / `WebSearch` / `WebFetch(domain:...)` が含まれていた。`ANTHROPIC_API_KEY` が action 環境にあるため、prompt injection 経由でこれらのツールが secret 探索・外部送信の経路になり得る。

対応:
- 汎用 Bash 探索ツール（`find` / `cat` / `grep` / `ls`）を allowedTools から削除。`Read` と事前収集された `CONSISTENCY_CONTEXT_PATH` のみで一貫性チェックを行う設計に変更した（前節「### 2-pass レビュー」の Phase 2 は既にこのファイルを Read する前提だったため、Claude 側の追加探索手段を断つだけで機能低下は限定的）
- `WebSearch` / `WebFetch`（pkg.go.dev / github.com / raw.githubusercontent.com）を allowedTools から削除。外部ドキュメント参照の利便性より、prompt injection 経由の外部送信経路を遮断することを優先した
- job env に `CLAUDE_CODE_SUBPROCESS_ENV_SCRUB: "1"` を追加。`Bash` tool 等の subprocess から ANTHROPIC_API_KEY 等の secret を scrub する defense-in-depth（claude-code-action がサポートする機能。導入は v2.1.83 以降の Claude Code CLI が前提）

既知の限界（残存リスク）:
- `CLAUDE_CODE_SUBPROCESS_ENV_SCRUB` は `Bash` 等の subprocess 経由のみが対象。`Read` tool は同じ sandbox 隔離の対象外で、`/proc/self/environ` 等への直接アクセスは別経路として残る（Microsoft Threat Intelligence が発見し、Anthropic が Claude Code v2.1.128 で `/proc` 配下の機微ファイルへのアクセスをブロックして緩和した既知の脆弱性）。今回の対応は Bash 経路の縮小に留まり、Read tool 経由の完全な防御は action 側 / CLI 側のバージョン追従に依存する
- Wiz custom rule `CIWorkflow-020` は `pull_request` トリガー単独でも `--allowedTools` に Bash / WebFetch / WebSearch 等の危険ツールが含まれていると fire する（2026-06-03 `WinTicket/security-check-db` PR #24 で確認済み、SKILL.md Gotchas 参照）。今回の縮小は Wiz ルールの実測結果とも整合する

### P1: display_report: true が untrusted content を露出

`display_report: true` は Claude の出力（PR diff・description の引用を含み得る）を GitHub Actions Step Summary に出力する。P0 の allowedTools 縮小後も、Step Summary は誰でも閲覧可能な面のため `display_report: false` に変更した。PR への投稿は SKILL.md の投稿ルール（`gh pr comment` / inline comment）で完結するため、機能低下はない。

### P2: Dart 対応の追加

`Prepare consistency context` step の test ペア検出・symbol 抽出が Go / TS / JS / TSX / JSX / Python のみで Dart を見ていなかった（`ghas-notify` は Dart リポ）。test ペアは `${base}_test.dart`、symbol は `class` / `enum` / `mixin` / `extension` / `typedef` とトップレベル `Future<...>` 関数を抽出するよう追加した。今後の言語追加（Rust / Swift / Kotlin 等）も同じ `case "$ext" in ... esac` パターンで追加できる。

## デバッグのヒント

Claude が「何も投稿せずに完了」する場合のチェックリスト:

1. action バージョンが v1.0.108 か（SHA `11a9dadd...`）
...
- v1.0.94 以降のセキュリティ機能で PR branch と main の workflow 差分をチェックしている
