...
- bot PR は `github.event.pull_request.user.type == 'User'` の job-level `if` で成功スキップし、人間レビューに委ねる
- `allowed_bots`: default (空) で運用。Wiz rule `459cd9a5` (HIGH) で `dependabot` / `renovate` 双方が Confused Deputy 攻撃ベクトルとして検出されるため両方とも入れない
