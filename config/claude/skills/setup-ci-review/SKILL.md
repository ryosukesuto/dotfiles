...
- **最終判定 (approve/request-changes) は workflow 側で強制実行**: 過去の実測で Claude がサマリコメント投稿後に `gh pr review --approve/--request-changes` を省略してタスク完了と判断するケースが再発。SKILL.md の文言強化だけでは防げなかったため、`claude-review.yml` に `always()` 条件の `Finalize PR review decision` post-step を配置し、サマリコメントの `P0: N件 / P1: N件` をパースして workflow 側で最終判定を打つ方式に切替済み。Claude 側の責務はサマリコメント投稿までとし、SKILL.md にもその旨を明記している。件数フォーマット（`P0:\s*\d+` / `P1:\s*\d+`）は parser が依存するため変更禁止
