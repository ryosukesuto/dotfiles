...
- `--allowedTools` には `mcp__github_inline_comment__create_inline_comment` を必ず含める。これが無いと Claude が分析だけして何も投稿せず終わるケースがある（`display_report: false` のため workflow summary にも出ない。PRコメントへの投稿がレビュー結果を確認する唯一の経路になる）
...
- **2-pass レビュー（B+C 案）**: `claude-review.yml` の `Prepare consistency context` step が `.claude-review-context.md` を生成し、 prompt の `CONSISTENCY_CONTEXT_PATH` で参照させる。SKILL.md template の「2-pass 順序」セクションで Phase 1 (logic/security) → Phase 2 (consistency) の順序を強制し、Phase 2 で `CONSISTENCY_CONTEXT_PATH` を Read する。Greptile 選択時もこの context 生成は走らせて構わない（害はない、むしろ補助情報として活用できる）。A 案（matrix で 2 セッションに分割）は ops / server-config 等のクリティカルリポでのみ別 workflow として後付けする。Claude 側に `Bash(grep/find/cat/ls)` を渡していないため、Phase 2 は `CONSISTENCY_CONTEXT_PATH` の事前収集内容のみで判定する（対象外の追加探索はできない設計。背景は `reference.md` の「allowedTools 縮小の経緯」参照）
- **`--allowedTools` は最小権限（Bash 探索系・WebSearch・WebFetch を含まない）**: `find` / `cat` / `grep` / `ls` の汎用 Bash と `WebSearch` / `WebFetch` は `--allowedTools` に含めない。ANTHROPIC_API_KEY が action 環境にあるため、prompt injection 経由でこれらのツールが secret 探索・外部送信の経路になり得る（Wiz custom rule `CIWorkflow-020` の実測結果とも整合）。一貫性チェックは Phase 2 の事前収集 context のみで行い、外部ドキュメント参照は行わない設計に変更した。詳細は `reference.md` の「allowedTools 縮小の経緯」参照
...
- **CIWorkflow-020 対応には `--disallowedTools` も必須**: Wiz custom rule `CIWorkflow-020` (`Workflow should not use AI agents with dangerous tools on external triggers`, MEDIUM) は `anthropics/claude-code-action` の `--allowedTools` に Bash / WebFetch / WebSearch 等の危険ツールが含まれていると、`pull_request` トリガー単独でも実測で fire する（2026-06-03 `WinTicket/security-check-db` PR #24 で確認）。ルール説明は `issues` / `issue_comment` / `pull_request_target` を例示するが、実装は `pull_request` も外部入力トリガーとして扱う模様。対応として skill template は以下を両方適用する: (1) `issue_comment` トリガーを採用しない（Poisoned Pipeline Execution 経路の遮断と兼ねる）、(2) `--disallowedTools "Edit,Write,NotebookEdit,Bash(git commit:*),..."` で危険ツールの surface area を明示剥離する。手動再レビューは空コミット push で代替する
...
- **`CLAUDE_CODE_SUBPROCESS_ENV_SCRUB: "1"` は job env に設定済み**: `Bash` tool 等の subprocess から ANTHROPIC_API_KEY 等の secret を scrub する defense-in-depth。ただし `Read` tool は対象外（`/proc/self/environ` 等への直接アクセスは別経路）。この変数だけで secret 露出経路を閉じたと誤解しないこと。詳細は `reference.md` の「allowedTools 縮小の経緯」参照
