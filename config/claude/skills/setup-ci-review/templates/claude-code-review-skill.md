...
> workflow の `--allowedTools` で列挙されたものに加えて、Anthropic の base GitHub tools
> （Read / Bash の一部 / mcp_github_inline_comment 等）が常時含まれる。`--disallowedTools`
> で Edit / Write / NotebookEdit / commit・push 系 git サブコマンドを明示的に落としているため、
> CI 上の Claude は読み取り + コメント投稿のみ可能で、ワークスペースは改変できない。
>
...
>
> 運用前提: 自動レビューは `pull_request` イベント（opened / synchronize / reopened /
> ready_for_review）のみで発火する。`issue_comment` トリガー（@claude メンション）は
> 採用しない。理由は GHAS CodeQL（Untrusted Checkout TOCTOU / Checkout of untrusted code）
> および Anthropic security docs が指摘する Poisoned Pipeline Execution 経路（issue_comment +
> gh pr checkout で attacker-controlled な PR head を trusted context = secrets 付きジョブに
> 持ち込む）を発生させないため。再レビューが必要な場合は新コミットを push（synchronize
> イベント）すること。
...
補正テーブルの「絶対 P0」と「文脈依存 P0」の境界は、その変更が単独で **対象スコープ全体（プロジェクト・組織・データ）の読み書き・削除・課金・他 IAM 変更** を成立させるかで判定する。成立するなら絶対、データの一部取得や限定的な実行に留まるなら文脈依存。

判別軸:

- 公開プリンシパル (`allUsers` / `allAuthenticatedUsers`) への管理系 role 付与（`*.admin` / `owner` / `editor` / `iam.securityAdmin` / `resourcemanager.projectIamAdmin` 等） → 絶対 P0
- 公開プリンシパルへのデータプレーン用 invoker / viewer / 限定 accessor → 文脈依存 P0
- 認証情報（API key / DB password / Service Account 鍵 / JWT 秘密鍵）のソースコード・ログへのコミット → 絶対 P0
- SSRF / SQL injection / RCE / Path traversal につながる入力検証の欠落 → 絶対 P0
- PII / 個人情報の認証なしエンドポイントへの露出 → 絶対 P0（role 種別を問わず）
- 認証なしの API エンドポイントで公開してよい情報のみ返す前提 → 文脈依存 P0
- 広めだが限定スコープな IAM（プロジェクト単位・dev 環境専用などの明示があるもの） → 文脈依存 P0

#### 判別に迷ったら

「その変更単独で乗っ取り・データ全削除・全データ exfil が成立するか」「対象データに PII / 機密情報が含まれるか」を問う。どちらか yes なら絶対 P0。例えば `roles/storage.objectViewer` 単独なら文脈依存だが、対象 bucket が PII を保存している場合は「PII の認証なし露出」に該当して絶対 P0 に格上げ。データの機微度はファイル名・bucket 名・近傍コメント・terraform リソース名・PR 本文から推定する。PR 本文の主張だけで「PII を含まない」と確定できない場合は **絶対 P0 側に倒す**（誤って下げるリスクのほうが大きい）。
...
### 初回レビュー（`EVENT_ACTION` が `opened` / `ready_for_review` / `reopened`）
...
### owner / repo / commit hash の解決

サマリ body の commit URL、`gh api repos/{owner}/{repo}/...` コマンドで使う識別子は以下から取得する:

- `{owner}/{repo}`: 第一選択は `${GITHUB_REPOSITORY}` 環境変数（`owner/repo` 形式）。取れない場合は `gh repo view --json owner,name -q '.owner.login + "/" + .name'`
- `{commit_hash}`: 第一選択は `HEAD_SHA`（全イベントで workflow から渡される）。`HEAD_SHA` が未設定な場合のみ `AFTER_SHA`（synchronize 時のみ埋まる）にフォールバック。両方取れない場合は `git rev-parse HEAD` を実行
- `{commit_hash_short}`: 上記で解決した `{commit_hash}` の先頭 7 文字

`AFTER_SHA` は `synchronize` イベント以外（`opened` / `ready_for_review` / `reopened`）で空文字になるため第一選択にしない。`HEAD_SHA` を使えば全イベントで commit URL が壊れない。

placeholder（`{owner}/{repo}` のような波括弧表記）のまま投稿しない。CI 実行時に必ず解決する。

### インラインコメントの集約と行番号

集約ルール:
- 同ファイル内の複数箇所が **同テーマ・同根本原因** なら 1 コメントに集約する（例: 同一ファイル内の関数 A・B が両方 camelCase の命名規則違反 → 1 件に集約）
- テーマや根本原因が異なれば箇所ごとに分ける（例: 一方が命名・もう一方が型注釈の問題 → 2 件に分ける）

行番号の特定:
- 第一選択: `gh pr diff` 出力の hunk header (`@@ -L,N +L,N @@`) から +側の開始行を計算する
- 複数行にまたがる指摘は `mcp__github_inline_comment__create_inline_comment` の `start_line` / `line` で範囲指定する
- hunk header が確定できない・新規ファイルで行番号が一意に決まらない場合は、対象リソース block / 関数定義の **冒頭行** を `line` として指定する
- 行番号が完全に確定できず MCP tool が失敗する場合は、サマリの指摘節にファイル名と該当コード断片を貼って代替する（最終手段）

### サマリと指摘の関係

サマリ body の構造:
- 箇条書きは 1 行要約（例: `- [P2] 命名規則が既存と不整合`）
- 詳細は別ブロックで ERROR/WHY/FIX 形式の全文を残す（インラインコメントと内容が重複しても良い）
- 補正履歴がある場合は `#### 補正履歴` 節として独立させる
...
- **`issue_comment` トリガーは採用しない**: GHAS CodeQL（Untrusted Checkout TOCTOU / Checkout of untrusted code）と Anthropic security docs が指摘する Poisoned Pipeline Execution 経路を避けるため、自動レビューは `pull_request` イベントのみで発火する設計にしている。手動再レビューが必要な場合は新コミット push（force-push を含む synchronize イベント）で代替する
