...
全コメント投稿後、レビューの最終ステップとして **必ず** 以下のどちらかを実行する。
このステップをスキップすると GitHub 上にレビュー状態（REQUEST_CHANGES / COMMENT）が残らず、
...
- P0 / P1 の指摘がない場合（P2/P3 のみ、または指摘なし）:
  ```
  gh pr review {PR_NUMBER} --comment -b "AIレビュー完了。重大な問題は検出されませんでした。"
  ```

`--approve` は使わない。承認は人間のレビュアーに委ねる設計になっている。

### 実行順序（必ずこの順序）

1. トップレベルサマリコメントを投稿（履歴蓄積方式）
2. インラインコメントを投稿
3. `gh pr review --comment` または `--request-changes` で最終判定を投稿（省略禁止）
