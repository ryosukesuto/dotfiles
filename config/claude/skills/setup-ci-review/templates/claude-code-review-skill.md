...
- 公開プリンシパル (`allUsers` / `allAuthenticatedUsers`) への管理系 role 付与
  - `roles/owner`, `roles/editor`, `roles/iam.securityAdmin`, `roles/resourcemanager.projectIamAdmin`
  - `roles/cloudfunctions.admin`, `roles/run.admin`, `roles/storage.admin`, `roles/storage.objectAdmin`
- 認証情報（API key, JWT 秘密鍵, DB password, Service Account 鍵）のソースコード/ログへのコミット
- SSRF / SQL injection / RCE / Path traversal につながる入力検証の欠落
- PII / 個人情報の認証なしエンドポイントへの露出（role 種別を問わず）

#### 文脈依存 P0 (補正可) の典型例

- 公開プリンシパルへの **データプレーン用 invoker / viewer / 限定 accessor** 系 role
  - `roles/run.invoker`, `roles/storage.objectViewer`, `roles/iap.httpsResourceAccessor`
  - `roles/cloudfunctions.invoker`, `roles/pubsub.subscriber`
- 認証なしの API エンドポイント（公開してよい情報のみ返す前提）
- 広めだが限定的なスコープ IAM（プロジェクト単位・dev 環境専用と明示）
...
- `{commit_hash_short}`: `AFTER_SHA` の先頭 7 文字
