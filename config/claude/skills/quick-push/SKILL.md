...
スキャン0件は目視の代替にならない。日本語の人名（姓 + `さん`）・社内Slackチャンネル名・プロダクト固有の用語は上記パターンでは原理的に検出できない。publicリポへの commit では、gitleaks と private情報チェックが両方0件でも `git diff --cached` を必ず読んでから commit する。誤検知が増えて警告が形骸化するため、これらを正規表現側で拾う拡張はしない。

2026-08-06、skill に追記した Good / Bad の実例に社内Slackチャンネル名と同僚3名の実名を書いた状態で、両スキャンが0件を返した。staged diff を目視して commit 前に気付き、プレースホルダへ置換した。
