...
Phase 5: 作業ログ反映の案内 (work-log skill への接続)
```
...
`work-log` skill (作業完了をデイリーノート + Time Block チェック + Linear Issue に反映する skill) への明示的な接続を行う。

post-merge skill の責務は GitHub / Linear / git 状態の片付けに限定し、作業ログ記録は `work-log` skill に委ねる。直接 `echo ... >> daily.md` で済ませると、Time Block チェックや Linear Issue 反映の拡張機能を取りこぼす。

### 手順

1. Phase 4 完了報告を出したあと、続けて 1 行案内を出す:
   ```
   作業ログをデイリーノート / Time Block / Linear に反映するなら `/work-log` を起動して。
   ```

2. ユーザーが「お願い」「やって」「うん」など肯定的な返答を返した場合のみ、`work-log` skill を呼ぶ。post-merge から自動連鎖はしない (ユーザーが既に別の方法でログを残している場合や、ログ不要のケースがあるため)。

3. ユーザーが特に反応しなかった場合は何もしない。post-merge 単体のタスクとしては Phase 4 までで完了している。
...
- 作業ログを直接 `echo ... >> daily.md` で残さない。Phase 5 の案内を経由して `work-log` skill に委ねる。直接 echo すると Time Block チェックや Linear Issue 反映を取りこぼす
