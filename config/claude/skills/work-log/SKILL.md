...
粒度は「何にどれだけ時間を使ったか」が分かる長さに留める。稼働ログは請求根拠なので、判断の中身・提案の論点・議論相手の氏名列挙は書かない（それらは議事録と Obsidian 側の役割）。目安は1行1作業・1文。チケット番号・PR 番号・成果物名のような後から辿れる識別子は残してよい。

- Good: `Slackキャッチアップ + <トピック>方針スレへの返信案検討・ドラフト作成`
- Bad: `Slackキャッチアップ + <トピック>方針スレ（参加者3名の氏名を列挙）への返信案検討。<相手の提案>に乗る形で、<判定基準>と<進捗可視化の方針>を明文化する提案の返信文をドラフト`
...
2. 所要時間の自動算出（原則ユーザーに質問せず概算する）:

   **議事録作成の場合**: セッション内容・Obsidianノート・Google Calendar 参照結果から MTG の時間帯（例: `13:00-14:00`）を抽出し、`MTG時間 + 15分` を所要時間とする。「MTG本編 + 議事録整理」として計上する。60分MTGなら約1.25時間、30分MTGなら約0.75時間。MTG時間帯が特定できない場合のみ通常作業として扱う。
   ...
   **通常作業の場合**:
   ...
   - 算出値はそのまま採用し、完了報告で「基点タイムスタンプ + 算出値」を提示する。誤っていればユーザーが訂正できる

   **AskUserQuestion に落とす条件（例外）**: 以下いずれかに該当する場合のみ質問する。それ以外は概算値をそのまま使う。
   - 差分が480分超（8時間超）— 前日以前の作業ログしか無く、基点が明らかに壊れている
   - 差分が0分または負値 — 基点が現在時刻より未来（別セッションのバグ的な記録）
   - 議事録作成でMTG時間帯が特定できず、かつ直近タイムスタンプもない
3. `mcp__atlassian-mediphone__getConfluencePage(cloudId, pageId, contentFormat: "markdown")` で現在の本文を取得
4. 当月セクションのテーブル末尾（`小計:` 行の直前）に新規行を追加: `| YYYY-MM-DD(曜日) | 約Xh | <Phase 1の要約> |`
   - 議事録の行は `<会議名>（HH:MM-HH:MM）+ 議事録整理` に揃える。「議事録作成」は使わない
   - 内訳（`会議30分+議事録整理15分` 等）は時間帯と所要時間から復元できるので書かない。同じ語を括弧内と行末で重複させない
   - 月をまたぐ場合は `## YYYY年M月` セクションを新設し、その配下にテーブルと `小計:` 行を作る
5. 小計時間: 当月セクションの「小計: 約XX時間」の数値に今回の所要時間を加算するだけ（テーブル全体の再集計はしない。曖昧な時間表記のパースミスで既存値がズレるのを避けるため）。既存行の所要時間を訂正した場合は、その差分も同じ方針で加減算する
...
- Confluence稼働ログの所要時間は原則自動算出し、質問しない。議事録作成は `MTG時間 + 15分`、通常作業は「直近タイムスタンプからの差分」で概算する。基点が壊れている場合（480分超・0分以下）のみ AskUserQuestion で確認する。詳細は Phase 5-1 参照
