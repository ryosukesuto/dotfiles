...
ユーザーへの説明用には must-fix / should-fix / watch を全て出してよいが、`gh pr review` / `gh pr comment` で GitHub に投稿するときは must-fix のみ に絞る（理由: `feedback_pr_review_compact.md` メモリ参照）。

### 文体ガイド (親しみのある敬語、全投稿共通)

`gh pr review` の本文・`gh pr comment`・review thread のインラインコメント、いずれも以下のトーンで書く。敬語ベースだが堅くなりすぎない。AI っぽい装飾語（太字、抽象ラベル、格言調の締め）は使わない。

書き方のルール:

- 事実は控えめに置く: 「確認しておきました」「〜みたいです」のような断定しすぎない言い回し
- 依頼は提案調: 「〜していただけると安心です」「〜してもらえると助かります」「〜するほうが良さそうです」
- 命令形を避ける: 「〜してください」より「〜していただけると助かります」
- 文末を「〜です」で揃えすぎない。「〜と思います」「〜と助かります」を混ぜる
- レビュー過程の内部用語は書かない（「triage で〜」「公式スキーマを fetch して〜」など）
- 太字（`**...**`）・「結論として」「以下の通り」等の前置き・「〜することが望ましい」「〜が肝要」等の硬い言い回しは使わない

骨格・例:

```
# Approve 本文 (must-fix 0 件)
LGTM です！

[fact-check したことがあれば「念のため〜は確認しておきました」で 1-2 行]

[マージ前に通したい確認があれば「〜だけ一度通しておけると安心かなと思います」]

[フォロー観点があれば「〜みたいな話は、別 Issue で拾ってもらえると助かります」]
```

```
# must-fix 指摘 (review 本文 1 件分)
`config/managed-settings.json:24`

env キーが `DISABLE_FEEDBACK_COMMNAD` になっていてタイポしているみたいです。
公式 schema 上は `DISABLE_FEEDBACK_COMMAND` なので、修正したほうが良さそうです。
```

```
# インラインコメント (review thread の単発コメント)
ここ、`null` が入りうるパスが見えていて、その時に下の `length` で NPE になりそうです。
前段で `?? []` あたり挟んでおくと安心かなと思います。
```

参考: 2026-05-15 PR #5 の Approve 文面 (`github.com/WinTicket/claude-code-policy#5`) が基準。

### must-fix が 1 件以上ある場合
...
### must-fix が 0 件の場合（Approve コメント）

should-fix / watch を本文に書かない。代わりに以下のいずれか:

- コードに変更してほしい修正がない: 「LGTM です！差分全体を見てマージブロック級の問題は無さそうです。」程度の 1-2 行
- author に運用フォロー / 別 Issue 対応を依頼したい指摘がある: 「以下の点はマージ後のフォローとして別 Issue で拾ってもらえると助かります」と前置きしてから、指摘の見出し + 1-2 行の補足のみ書く。詳細な修正案は書かない（運用負荷の本筋を残し、PR コメント量を抑える）

文体は上の「文体ガイド」節に従う。

### 投稿前チェック

1. 本文に `should-fix` `watch` の文字列が残っていないか grep する
2. 「別観点の指摘」（dedupe で削った代替視点）のセクションを削除する
3. 同じ HEAD SHA に対する自分のレビューが既に存在しないか `gh api repos/$REPO/pulls/$PR_NUM/reviews` で確認
...
300行以上の変更があるPRでは、先入観なしのレビュー観点を追加することで、著者バイアスによる見落としを検出できる。
Codexではユーザーが「並列」「マルチエージェント」「subagentで」などを明示した場合のみ、Codex subagentに委譲する。明示がない場合は、自分でPR説明を閉じた状態のdiff-onlyパスとして同じ観点を実行する。

例外: 自分自身がsubagent内でこのskillを実行している場合、Reader Testingの追加委譲は発動しない。subagentの再帰利用を避けるため。

```
Codex subagent に渡す場合:
