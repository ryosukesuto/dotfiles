...
- このskillはClaude互換のため `config/claude/skills` 配下に置かれているが、実行時のskillパスは `~/.agents/skills` を優先する。
...
if [ ! -f "$REVIEW_PR_REFERENCE" ] || [ ! -f "$TRIAGE_SCRIPT" ]; then
...
for reviewer in $REVIEWERS; do
  python3 "$REVIEW_ORCH_DIR/scripts/build-reviewer-prompt.py" \
...
- `uv run` がキャッシュ書き込みやsandboxで落ちる場合、`python3 -c "import yaml"` が通る環境では `python3 "$TRIAGE_SCRIPT"` に切り替える。orchestrator配下の `build-bundle.py` / `build-reviewer-prompt.py` / `normalize.py` / `report.py` はpython3直実行でよい。
