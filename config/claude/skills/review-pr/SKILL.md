...
# Step 4b: ユーザーへ返す前にPRレビュー向け文体を機械確認
if ! python3 ~/.claude/skills/proofread/check.py \
  "$REPORT_MD" --pr-review --json | jq -e '
  .severity_terms == 0 and
  .internal_process == 0 and
  .strong_directive == 0 and
  .space_before_emoji == 0 and
  .bold_emphasis == 0 and
  .missing_request_emoji == 0 and
  .emoji_not_at_end == 0
' >/dev/null; then
  echo "ERROR: review report contains author-facing tone violations. Fix the finding draft and regenerate the report." >&2
  exit 1
fi
...
report.py の出力はツール結果に流れるだけで、ユーザーには届かない。最終テキストメッセージで文体チェック済みの`$REPORT_MD`全文をそのまま貼り付けてユーザーに伝える。要約に圧縮せず、全findingsを掲載する。
...
5. reference.mdの出力フォーマットで、各指摘に詳細説明・根拠・修正の方向・レビュー草案を付ける
6. ユーザーへ返す前に`proofread`の`pr-review`モードを適用し、内部用の優先度やレビュー過程が残っていないことを確認する
...
- **GitHubに投稿するレビュー本文は、内部判定でマージ前対応が必要な指摘だけに絞る**。補足や別観点のセクションは削除し、内部の区分名も本文へ出さない。対象が0件なら投稿しないか、「特に懸念なし、フォロー観点はチャットで共有済」程度の短いコメントだけにする。マージ後に拾いたい内容がある場合は、別Issue起票のお願いを1〜2文だけApprove bodyへ添える。詳細は`feedback_pr_review_compact.md`メモリ参照
