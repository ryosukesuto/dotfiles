...
### 2. Codex起動 + 自分のdiff分析（並列実行）

Codexの応答待ち時間を活用して、自分のdiff分析を先に進める。
...
# ここでは wait_response しない → 2c に進む
```

`$PANE_MGR ensure` が失敗した場合（tmux/cmux外）のみ `codex exec` にフォールバック（この場合は直列実行になる）:

```bash
codex exec "{上記で組み立てたプロンプト}"
```

#### 2c. 自分のdiff分析（Codex応答待ちと並列）

Codexが処理している間に、自分でdiffを読んでレビューを進める:
- 変更ファイルを読み、P0-P3で問題を洗い出す
- テストカバレッジ、エラーハンドリング、セキュリティの観点で分析
- 新規ディレクトリがあれば追加チェック（後述）を実施

#### 2d. Codex応答の回収

自分の分析が終わったら、Codexの応答を回収する:
...
Codex分析、あなた自身の評価（2cの結果）、既存レビューコメントを統合し、以下のフォーマットで出力。
