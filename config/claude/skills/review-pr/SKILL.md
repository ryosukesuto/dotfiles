...
**事前準備 2: 既存レビュー / 依存 Issue 状態の取得（subagent 起動前に必ず実行）**

reviewer 起動後にユーザーから「Claude Code Action がもう APPROVED している」「依存 Issue は Done になっている」と指摘されて findings が崩れるケースが頻発する。subagent に渡す前に以下を取得し、reviewer prompt 末尾に追記する。
...
gh pr view $PR_ARGS --json body,commits 2>/dev/null \
  ...
  > "${WORK}-referenced-issues.txt"
```

抽出した Issue ID の状態は `mcp__linear-server__get_issue` で確認する。Done であれば「依存 Issue 完了済み」、In Progress なら「未完了に依存」とラベル付けして reviewer prompt に共有する。これで「依存 Issue 完了済みなのに『apply 失敗リスク』を should-fix で出す」類の手戻りを減らせる。
...
- レビュー結果はユーザーへの出力として返すだけで終了する。`gh pr review` / `gh pr comment` 等で GitHub に投稿するかどうかはユーザーが判断する。**ユーザーから明示的に「投稿して」と指示されるまで書き込み操作は行わない**。投稿指示を受けた時点で**必ず**以下4点を順に確認すること（チェックを忘れて投稿してから訂正が入る事象が複数回発生済み）:
  1. 投稿本文は must-fix のみに絞ったか（下記 must-fix 絞り込みルール参照）
  2. 投稿本文を `proofread` skill で校正したか。author 向け生成物の AI っぽさ / トーン違反を初稿の段階で取り除く目的。トーン訂正の往復削減のため、最初の PATCH/POST 前に必ず通す（詳細は `feedback_pr_comment_tone.md` の How to apply）
  3. 同 PR の直近レビューに自分の user × 60 秒以内 × 同 commit_id が無いか（race condition 二重投稿防止ルール参照）
  4. 書き込みコマンドに `|| fallback` / リトライを付けていないか（二重投稿防止ルール参照）
- subagent / Claude bot / Codex / Greptile から上がってきた findings を should-fix 以上の severity で最終出力に通す前に、以下3点を裏取りする。1 点でも未確認なら severity を watch に下げる、または「server 挙動次第」等の前提条件を明記して提示する。詳細と過去事故は `feedback_pr_review_factcheck.md` メモリ参照:
...
- **GitHub に投稿するレビュー本文は must-fix のみに絞る**。should-fix / watch / 「別観点の指摘」セクションは削除してから投稿する。`report.py` 出力（ユーザー説明用）と GitHub 投稿用は別物として扱う。must-fix が 0 件のときは投稿しない、または「特に懸念なし、フォロー観点はチャットで共有済」程度の短いコメントだけにする。author に伝えたい should-fix / watch がある場合は別 Issue 起票を依頼する 1-2 行を Approve body に添える（指摘の中身は書かない）。詳細は `feedback_pr_review_compact.md` メモリ参照
- オーケストレーターパスで `uv run python3 ~/.claude/skills/review-orchestrator/scripts/*.py` が `error: failed to open file '~/.cache/uv/sdists-v9/.git': Operation not permitted` で落ちる sandbox 環境では、以下のいずれかで対処する。Claude Code が Bash tool 経由で叩く場合は (a) が簡便:
  - (a) `uv run` を使う Bash 呼び出しに `dangerouslyDisableSandbox: true` を付ける（uv は `~/.cache/uv` への書き込みが必須なので review-pr の triage / orchestrator step では一律 sandbox を外す前提で組む）
  - (b) `python3 ~/.claude/skills/review-orchestrator/scripts/*.py` の **直接実行に切り替える**。`build-bundle.py` / `build-reviewer-prompt.py` / `normalize.py` / `report.py` はいずれも追加パッケージ不要なので python3 直で動く。`triage.py` のみ pyyaml 依存があるため、`python3 -c "import yaml"` が通れば直実行、通らなければ `uv run --with pyyaml` + sandbox 無効化を維持する
- 中間ファイルパスは `$TMPDIR` をシェルで展開せず、固定 path にする。`dangerouslyDisableSandbox: true` を付けた Bash 呼び出しと sandbox 有効の Bash 呼び出しで `$TMPDIR` が別解決される（前者は `/var/folders/.../T`、後者は `/tmp/claude-xxx`）ため、片方で書いて片方で読むと「No such file or directory」になる。対策: `WORK="$HOME/.cache/review-pr/${PR_NUM}"` のように HOME 配下の固定 path を使い、`mkdir -p "$HOME/.cache/review-pr"` で先に確保する。または同一 Bash 呼び出しに閉じる場合のみ `$TMPDIR` を許容する（2026-05-15 PR #7194 で発生、cp で復旧した）
