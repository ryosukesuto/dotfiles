...
4. **In Progress 棚卸しトリガー**: Phase 2 で取得した assignee=me の In Progress Issue が 10 件超ある場合、本人のフォーカス分散リスクとして提示。「Backlog化候補」「他者assigneeへ譲渡候補」「実態としてはWaiting/Blockedの候補」をユーザーに確認して縮減を促す。閾値は 10 件 (経験則。2026-05 月次レビューで 15 件まで膨張していた事例から設定)
5. 承認後、`save_issue` でCycle割り当てを変更
...
2. 結果を `jq` で整形して、リポごとに以下を抽出:
   ...
   - **worktree 誤検出を除外**: `.worktrees/` を含むパスはローカル worktree (実 GitHub リポではない) のため対象外。`jq 'map(select(.repo | contains(".worktrees/") | not))'` で除外する
3. **既存 Issue 一括検索**を Linear に対して行う。リポごとに `query: "setup-ci-review {repo_name}"` で個別に叩くと往復が増えるため、まず広めに `query: "setup-ci-review"` で 1 回叩いて結果を取得し、ローカルで repo 名マッチングする
   ```
   mcp__linear-server__list_issues(query: "setup-ci-review", limit: 50)
   mcp__linear-server__list_issues(query: "claude-code-review 導入", limit: 30)
   ```
   重複 Issue 検知: 同じリポに対して複数 Issue (Backlog 含む) が存在する場合は新規起票せず、ユーザーに「どちらを生かすか、片方を Cancel するか」を確認。今回 (2026-05) PF-1903 (ops, 導入済リポに残存) / PF-1904 vs PF-2110 (server-config 重複) を後追いで発見した経緯あり
