...
## subagentプロンプト（Phase 2: プロジェクト横断タスク取得）

```
Linearから自分がアサインされている全プロジェクトとタスクを取得し、整形して返してください。

## 手順

1. ToolSearchで Linear MCPツールをロード:
   ToolSearch(query: "+linear projects issues list cycles")

2. 自分が参加しているプロジェクトを取得:
   mcp__linear-server__list_projects(member: "me", state: "started")

3. 各プロジェクトのIssueを取得:
   mcp__linear-server__list_issues(assignee: "me", project: <projectId>)

4. プロジェクトに属していない自分のIssueも取得:
   mcp__linear-server__list_issues(assignee: "me", limit: 100)
   → プロジェクト付きIssueのIDを除外して「プロジェクト外Issue」リストを作る
...
- projectId
- projectName
- issues: [{identifier, title, status, priority, estimate, description(冒頭200文字), URL}]

### プロジェクト外
- issues: [{identifier, title, status, priority, estimate, description(冒頭200文字), URL, createdAt}]
```

## subagentプロンプト（Phase 5: Current Cycle取得）
