...
|---------|-----------|----------------|-----------|
| `gws` | WinTicket（会社） | winticket-workspace | `~/.config/gws/` |
| `gws-personal` | ryosuke.suto@gmail.com | plasma-creek-116906 | `~/.config/gws-personal/` |
...
- OAuthクライアント: GCPプロジェクト `plasma-creek-116906` で作成済み（デスクトップアプリ型）
...
- カレンダーID: `iapurvt3lvrmqb8qtbrj5apepn5902uh@import.calendar.google.com`
- `+agenda` ヘルパーは `--calendar` 指定で使う
- `primary` カレンダーには勤務場所（オフィス/リモート）のみ登録されている

今日の予定:
```bash
eval "$(mise activate zsh)" && gws calendar +agenda --today --calendar 'S32943 @ NASCA Calendar'
```

今週の予定:
```bash
eval "$(mise activate zsh)" && gws calendar +agenda --week --format table --calendar 'S32943 @ NASCA Calendar'
```

N日間の予定:
```bash
eval "$(mise activate zsh)" && gws calendar +agenda --days 3 --calendar 'S32943 @ NASCA Calendar'
```

任意の日付の予定（`+agenda` は日付指定不可のため API 直接呼び出し）:
```bash
eval "$(mise activate zsh)" && gws calendar events list --params '{"calendarId": "iapurvt3lvrmqb8qtbrj5apepn5902uh@import.calendar.google.com", "timeMin": "YYYY-MM-DDT00:00:00+09:00", "timeMax": "YYYY-MM-DDT00:00:00+09:00", "singleEvents": true, "orderBy": "startTime"}'
```
