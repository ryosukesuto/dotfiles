...
#### 🏗️ 保守性・可読性（P2-P3優先）
- [ ] **設計の質**: 責務分離が適切か（単一責任原則）
- [ ] **関数の複雑度**: 関数が適切な長さか（50行以内）、ネストが深すぎないか（3階層以内）
- [ ] **重複の排除**: 重複コードがないか（3回以上の繰り返しは共通化）
- [ ] **命名規則**: 変数・関数名が意図を明確に表現し、プロジェクト全体で統一されているか
- [ ] **リーダブルコード**: 複雑なロジックにコメント、マジックナンバーの定数化、早期リターン活用
- [ ] **ドキュメント整合性**:
